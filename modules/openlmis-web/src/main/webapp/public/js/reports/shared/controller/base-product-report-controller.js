function BaseProductReportController($scope, $filter, ProductReportService, $cacheFactory, $timeout, FacilityService, GeographicZoneService, $dialog, DateFormatService, $location, messageService, HomeFacilityService) {
  ...
  $scope.getProvincesAndDistricts = function (geographicZoneLevels, data) {
    ...
    if (!_.includes(['/over-stock', '/stock-on-hand-all-products', '/expired-products', '/expiring-products', '/lot-expiry-dates'], $location.$$path)) {
  ...
  $scope.pickMVs = function(period) {
    if (period.periodStart.getTime()  >= new Date("2021/07/21").getTime()){
      return 'vw_period_movements_after_20210721';
    }else {
      return 'vw_period_movements_before_20210721';
    }
  };
