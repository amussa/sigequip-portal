...
public class RestLoginService {
  ...
  public LoginInformation login(String username, String password) {
    ...
    if (appInfo != null && appInfo.getUniqueId() != null && uniqueId != null &&
        !StringUtils.equals(uniqueId, appInfo.getUniqueId())) {
      throw new DataException(
          "One facility can only sign in on one device. Your facility has signed in on another device.");
    }
  ...
  private LoginInformation getLoginInformation(String username) {
    ...
    List<String> programs = FluentIterable.from(getProgramsSupportedByFacilityId(facilityId))
        .transform(new Function<ProgramSupported, String>() {
          @Override
  ...
  public List<FacilitySupportedProgram> getFacilitySupportedPrograms(Long facilityId) {
    return FluentIterable.from(getProgramsSupportedByFacilityId(facilityId))
        .transform(new Function<ProgramSupported, FacilitySupportedProgram>() {
          @Override
