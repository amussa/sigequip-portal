...
CREATE MATERIALIZED VIEW vw_lot_expiry_dates_before_20211001 AS
   SELECT
   ...
   FROM
     ...
     WHERE stock_card_entry_key_values.keycolumn LIKE 'LOT#%' AND  stock_card_entries.createddate <= '2021-10-01 23:59:59'::timestamp without time zone
 ...
 CREATE MATERIALIZED VIEW vw_lot_expiry_dates_after_20211001 AS
   SELECT
     uuid_in(md5(random() :: TEXT || now() :: TEXT) :: cstring) AS uuid,
...
CREATE UNIQUE INDEX idx_vw_lot_expiry_dates_before_20211001 ON vw_lot_expiry_dates_before_20211001 (stock_card_entry_id, uuid);

CREATE INDEX idx_vw_lot_expiry_dates_before_20211001_prc_dtc_fcc ON vw_lot_expiry_dates_before_20211001(province_code, district_code, facility_code);

CREATE OR REPLACE FUNCTION refresh_vw_lot_expiry_dates_before_20211001()
  RETURNS INT LANGUAGE plpgsql
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY vw_lot_expiry_dates_before_20211001;
  RETURN 1;
END $$;

CREATE UNIQUE INDEX idx_vw_lot_expiry_dates_after_20211001 ON vw_lot_expiry_dates_after_20211001 (stock_card_entry_id, uuid);

CREATE INDEX idx_vw_lot_expiry_dates_after_20211001_prc_dtc_fcc ON vw_lot_expiry_dates_after_20211001(province_code, district_code, facility_code);

CREATE OR REPLACE FUNCTION refresh_vw_lot_expiry_dates_after_20211001()
  RETURNS INT LANGUAGE plpgsql
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY vw_lot_expiry_dates_after_20211001;
  RETURN 1;
END $$;